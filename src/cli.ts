...
import {
	createCommitAction,
	type CommitCommandRuntime,
} from "./commands/commit.js";
import {
	createConfigCommand,
	type CommandRuntime as ConfigCommandRuntime,
...
const COMMAND_NAMES = new Set(["init", "config", "doctor", "split"]);
...
function applyRootOptions(command: Command): Command {
	return applyExecutionOptions(command)
		.option(
			"--provider <provider>",
			"Override the AI provider for this commit workflow.",
		)
		.option(
			"--model <model>",
			"Override the AI model for this commit workflow.",
		)
		.option(
			"--base-url <baseUrl>",
			"Override the OpenAI-compatible base URL for this commit workflow.",
		)
		.option(
			"--prompt-style <promptStyle>",
			"Override the commit prompt style for this commit workflow.",
		)
		.option(
			"--custom-prompt <customPrompt>",
			"Override custom prompt instructions for this commit workflow.",
		);
}
...
export function buildCli({
	...
	const splitCommand = createSplitCommand({ ...streams });
	...
	applyExecutionOptions(splitCommand);
