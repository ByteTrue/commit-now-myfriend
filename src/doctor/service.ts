...
function readIssueCodes(issues: DoctorIssue[], check: DoctorIssue["check"]): string[] {
  return issues.filter((issue) => issue.check === check).map((issue) => issue.code);
}

function summarizeCheck(issues: DoctorIssue[], check: DoctorIssue["check"]): DoctorCheckStatus {
...
function toSnapshot(config: ConfigValues | EffectiveConfig | null): DoctorConfigSnapshot | null {
  if (!config) {
    return null;
  }
...
function resolveEffectiveConfigSources(
  ...
  return {
    apiKey: resolveConfigSource("apiKey", userConfig, projectConfig, envConfig, effectiveConfig),
    baseURL: resolveConfigSource("baseURL", userConfig, projectConfig, envConfig, effectiveConfig),
    model: resolveConfigSource("model", userConfig, projectConfig, envConfig, effectiveConfig),
    provider: resolveConfigSource("provider", userConfig, projectConfig, envConfig, effectiveConfig)
  };
}
...
async function runGitVersion(cwd: string, env?: NodeJS.ProcessEnv): Promise<GitVersionResult> {
  try {
...
function buildGitCheck(issues: DoctorIssue[], gitVersion: GitVersionResult): DoctorGitCheck {
  if (!gitVersion.available) {
    pushIssue(issues, {
      check: "git",
      code: "git_executable_missing",
      message: "Git is not available on PATH; install git before using cnm workflows.",
      severity: "error"
    });
  }
...
async function inspectDirectory(candidatePath: string): Promise<DirectoryAccessResult> {
  try {
...
async function safeLoadUserConfig(cwd: string, env?: NodeJS.ProcessEnv): Promise<SafeConfigSourceResult> {
  ...
  try {
    ...
    return {
      config: {},
      error: error instanceof Error ? error : new Error("Unable to load user config."),
      exists: metadata.exists,
      warningMessages: []
    };
  }
...
async function safeLoadProjectConfig(cwd: string, env?: NodeJS.ProcessEnv): Promise<SafeConfigSourceResult> {
  const paths = getConfigPaths({ cwd, env });
  ...
  try {
    ...
    return {
      config: {},
      error: error instanceof Error ? error : new Error("Unable to load project config."),
      exists: metadata.exists,
      warningMessages: []
    };
  }
...
function safeLoadEnvConfig(env?: NodeJS.ProcessEnv): SafeConfigSourceResult {
  try {
    ...
    return {
      config: {},
      error: error instanceof Error ? error : new Error("Unable to read environment configuration."),
      exists: true,
      warningMessages: []
    };
  }
...
function buildRepositoryCheck(issues: DoctorIssue[], inspection: Awaited<ReturnType<typeof inspectGitRepository>> | null): DoctorRepositoryCheck {
  if (!inspection) {
    return {
      branchName: null,
      ...
      rootPath: null,
      status: summarizeCheck(issues, "repository") === "pass" ? "warning" : summarizeCheck(issues, "repository")
    };
  }
...
function buildConfigFileCheck(
  ...
  if (status === "warning") {
    const firstWarning = issues.find((issue) => issue.check === check && issue.severity === "warning");
    message = firstWarning?.message ?? message;
  }

  if (status === "error") {
    const firstError = issues.find((issue) => issue.check === check && issue.severity === "error");
...
function appendEffectiveConfigIssues(
  ...
  if (missingExplicitProviderConfig) {
    pushIssue(issues, {
      ...
      message: "Provider configuration is still using built-in defaults; run `cnm init` or configure cnm before generating commits.",
      severity: "error"
    });
  }
  ...
  if (effectiveConfig.provider === "openai-compatible" && !effectiveConfig.baseURL) {
    pushIssue(issues, {
      check: "effectiveConfig",
      code: "provider_config_missing",
      message: "The openai-compatible provider requires `baseURL` to be configured.",
      severity: "error"
    });
  }
}
...
function buildEffectiveConfigCheck(
  ...
  if (status === "warning") {
    const warning = issues.find((issue) => issue.check === "effectiveConfig" && issue.severity === "warning");
    message = warning?.message ?? message;
  }

  if (status === "error") {
    const error = issues.find((issue) => issue.check === "effectiveConfig" && issue.severity === "error");
...
function buildGuidance(reportIssues: DoctorIssue[], userConfigPath: string): string[] {
  const guidance = new Set<string>();
  const issueCodes = new Set(reportIssues.map((issue) => issue.code));

  if (issueCodes.has("provider_config_missing") || issueCodes.has("api_key_missing")) {
    guidance.add("Run `cnm init` or `cnm config set apiKey <value>` to finish provider setup. Prefer the `CNM_API_KEY` environment variable when possible.");
  }

  if (issueCodes.has("not_git_repository")) {
    guidance.add("Run `cnm doctor` from inside the repository you want to use with `cnm`.");
  }

  if (issueCodes.has("project_api_key_ignored")) {
    guidance.add("Remove `apiKey` from project config; project-level secrets are ignored by design.");
  }

  if (issueCodes.has("user_config_permissions_insecure")) {
    guidance.add(`On Unix-like systems, run \`chmod 600 ${userConfigPath}\` to tighten user config permissions.`);
  }

  guidance.add("This package installs the `cnm` binary from the `commit-now-myfriend` package name.");
  guidance.add("If `cnm` resolves to another executable, use `npx commit-now-myfriend`, `pnpm exec cnm`, or remove the conflicting global package.");
...
export async function runDoctor({
  ...
  const inspection = gitVersion.available ? await doctorDependencies.inspectGitRepository({ cwd, env }) : null;
  ...
  const configDirectoryCheck = buildConfigDirectoryCheck(issues, paths.userConfigHome, configDirectoryAccess);
  ...
  appendConfigSourceIssues(issues, "userConfig", userConfigResult, "user_config_invalid", "User config is invalid.");
  appendConfigSourceIssues(issues, "projectConfig", projectConfigResult, "project_config_invalid", "Project config is invalid.");
  maybeWarnOnPermissions(issues, "userConfig", paths.userConfigPath, userConfigMetadata.mode);
  ...
  appendEffectiveConfigIssues(issues, effectiveConfig, effectiveSources, envConfigResult);
  ...
  const effectiveConfigCheck = buildEffectiveConfigCheck(issues, effectiveConfig, effectiveSources);
  const summary = {
