...
test("createInstallerSessionPaths isolates archive and checksum files per run", () => {
	const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "cnm-install-session-"));
	const archiveName = "commit-now-myfriend_0.1.4_linux_amd64.tar.gz";

	const first = createInstallerSessionPaths(tmpDir, archiveName, "0.1.4");
	const second = createInstallerSessionPaths(tmpDir, archiveName, "0.1.4");

	assert.notEqual(first.sessionRoot, second.sessionRoot);
	assert.equal(first.archivePath, path.join(first.sessionRoot, archiveName));
	assert.equal(second.archivePath, path.join(second.sessionRoot, archiveName));
	assert.equal(
		first.checksumsPath,
		path.join(first.sessionRoot, "checksums-0.1.4.txt"),
	);
	assert.equal(
		second.checksumsPath,
		path.join(second.sessionRoot, "checksums-0.1.4.txt"),
	);
	assert.equal(first.extractDir, path.join(first.sessionRoot, "extract"));
	assert.equal(second.extractDir, path.join(second.sessionRoot, "extract"));
});
